...
export default function Sidebar() {
  ...
  return (
    <>
      <section className='flex flex-grow flex-col max-w-60 justify-between bg-primary text-primary-foreground p-4'>
            ...
            {sidebar.map((section, index) => (
              <ListboxSection key={index} title={section.title} showDivider classNames={{
                ...
                {section.items.map((item, itemIndex) => (
                  <ListboxItem key={itemIndex} className='hover:!bg-white px-4 py-2' startContent={<span className="material-symbols-outlined !text-xl">
                    {item.icon}
                  </span>}>
                    {item.label}
                  </ListboxItem>
